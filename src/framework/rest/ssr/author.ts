...
import { fetchAuthor, fetchAuthors } from '@framework/authors/authors.query';
...
export async function getStaticPaths({ locales }: GetStaticPathsContext) {
  const { data } = await fetchAuthors({
    queryKey: [API_ENDPOINTS.AUTHORS, { limit: 100 }],
  });
  const paths = data?.flatMap((author: any) =>
...
export const getStaticProps: GetStaticProps = async ({ params, locale }) => {
  invariant(params?.author, 'slug is required');
  ...
  try {
    const author = await fetchAuthor(params.author.toString());
    await queryClient.prefetchInfiniteQuery(
      [API_ENDPOINTS.PRODUCTS, { author: params.author }],
      fetchProducts,
      {
        staleTime: 60 * 1000,
      }
